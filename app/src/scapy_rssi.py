...
class ScapyRssi:
    ...
    def plot(self, num):
        ...
        with self.dataMutex:
            data = dict(self.data)
        nodes = [x[0] for x in
                 sorted([(addr, len(data[addr])) for addr in data.keys()], key=lambda x: x[1], reverse=True)]
...
if __name__ == "__main__":
    sniffer = ScapyRssi("wlp2s0")
    time.sleep(30)
    for k, v in dict(sniffer.data).items():
        print("{}: {}".format(k, v))
